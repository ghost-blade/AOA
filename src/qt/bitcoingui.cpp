    ...
    platformStyle(_platformStyle)
{
    ...
    if(fCheckForUpdates && qtumVersionChecker->newVersionAvailable())
    {
        ...
        QString message(tr("New version of Aurola wallet is available on the Aurola source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Aurola address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and aurola: URIs)"));
    ...
    QRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/arctoken"), tr("&ARC Tokens"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Aurola addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Aurola addresses"));
    ...
    openAction->setStatusTip(tr("Open a aurola: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Aurola command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Aurola network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Aurola"); // default title
